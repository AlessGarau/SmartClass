        ...
        <div className="w-full overflow-x-auto">
            ...
            {isLoading ? (
                ...
                <div className="min-w-[768px]">
                    <table className="flex flex-col justify-between items-center rounded-md bg-white border border-grayBorder border-solid border-2 w-full">
                        <thead className="flex flex-col gap-2 bg-lightGray w-full rounded-t-md p-4 font-semibold text-[12px]">
                            <tr className="w-full flex justify-between items-center">
                                <th className="flex-1 min-w-[150px]">CLASSE</th>
                                {currentWeek.map((day) => (
                                    <th key={day.day} className="flex-1 min-w-[120px]">
                        ...
                        <tbody className="w-full">
                            {weekPlanningData?.classrooms.map((classroom) => (
                                <tr key={classroom.id} className="flex justify-between items-center p-4 border-b border-gray-200">
                                    <td className="flex-1 font-medium min-w-[150px]">
                                    ...
                                    {currentWeek.map((day) => {
                                        ...
                                        return (
                                            <td key={day.day} className="flex-1 h-96 min-w-[120px]">
