...
const RoomsPage = () => {
    ...
    const { data: buildingData, isLoading: isLoadingBuildings } = useQuery(
        roomQueryOptions.buildingOptions()
    );
    const { data: roomsData } = useQuery(
        roomQueryOptions.getRooms({ ...appliedFilters, limit, offset })
    );
    const { data: roomsCountData } = useQuery(
        roomQueryOptions.roomCount(appliedFilters)
    );
        ...
        <div className="flex flex-col gap-4 p-4 sm:p-8 md:p-12 lg:p-20">
                ...
                actions={
                    <>
                        ...
                        <Button
                            ...
                            onClick={() => setAddRoomSectionVisible(false)}
                        />
                    </>
                }
            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="flex flex-col gap-2">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    handleNewRoomChange("name", e.target.value)
                                }
            ...
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
                <div>
                    ...
                    <div className="text-base sm:text-lg md:text-[20px]">
                        Gérer et contrôler l’état de vos salles
                    </div>
                </div>
            ...
            <FilterContainer>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full lg:w-auto">
                    ...
                    <Dropdown
                        ...
                        onSelect={(option) =>
                            handleChange("building", option.value as string)
                        }
                        disabled={isLoadingBuildings}
                    />
                </div>
            ...
            <div className="mb-2 flex justify-end text-gray-600">
                {roomsCountData?.data?.count !== undefined && (
                    <span>
                        {roomsCountData.data.count} salle
                        {roomsCountData.data.count > 1 ? "s" : ""} trouvée
                        {roomsCountData.data.count > 1 ? "s" : ""}
                    </span>
                )}
            </div>
            {Array.isArray(rooms) && rooms.length > 0 && (
                <RoomsContainer displayedRooms={rooms} />
            )}
