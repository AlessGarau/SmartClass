...
async function main() {
  ...
  const lessons = await database
    .insert(schema.lessonTable)
    .values([
      {
        title: "Algebra Basics",
        start_time: new Date("2024-01-15T08:00:00"),
        end_time: new Date("2024-01-15T09:30:00"),
      ...
      {
        title: "Newton's Laws",
        start_time: new Date("2024-01-15T13:00:00"),
      ...
      },
      {
        ...
        start_time: new Date("2024-01-15T14:45:00"),
        end_time: new Date("2024-01-15T16:15:00"),
        class_id: classes[3].id,
        room_id: rooms[3].id,
      },
      {
        title: "World War II",
        start_time: new Date("2024-01-16T08:00:00"),
        end_time: new Date("2024-01-16T09:30:00"),
      ...
      },
      {
        ...
        end_time: new Date("2024-01-16T11:30:00"),
        class_id: classes[5].id,
        room_id: rooms[7].id,
      },
      {
        title: "Python Programming",
        start_time: new Date("2024-01-16T13:00:00"),
        end_time: new Date("2024-01-16T14:30:00"),
        class_id: classes[6].id,
        room_id: rooms[4].id,
      },
      {
        title: "Cell Biology",
        start_time: new Date("2024-01-16T14:45:00"),
        end_time: new Date("2024-01-16T16:15:00"),
  ...
  // Seed pressure data
  const pressures = await database
    .insert(schema.pressureTable)
    .values([
      {
        id: 1,
        room_id: rooms[0].id,
  ...
  const movements = await database
    ...
    .values([
      ...
      },
      {
        ...
        room_id: rooms[6].id,
        data: "no_movement",
        sensor_id: sensors[14].id,
      },
    ])
    .returning();

  // Seed user-lesson relationships (teachers assigned to lessons)
  const userLessons = await database
    .insert(schema.userLessonTable)
    .values([
  ...
  const userMobileDevices = await database
    .insert(schema.userMobileDeviceTable)
    .values([
      {
        user_id: users[0].id,
        device_token: "token_john_123",
        refresh_token: "refresh_john_123",
      },
      {
        user_id: users[1].id,
        device_token: "token_mary_456",
        refresh_token: "refresh_mary_456",
      },
      {
        user_id: users[2].id,
        device_token: "token_david_789",
        refresh_token: "refresh_david_789",
      },
      {
        user_id: users[3].id,
        device_token: "token_sarah_012",
        refresh_token: "refresh_sarah_012",
      },
      {
        user_id: users[5].id,
        device_token: "token_admin_345",
        refresh_token: "refresh_admin_345",
      },
    ])
    .returning();
  ...
  const reports = await database
    ...
    .values([
      ...
      {
        equipment_id: equipment[15].id,
        description: "Heater making unusual noise",
        status: "pending",
      },
      {
        equipment_id: equipment[0].id,
        description: "AC maintenance completed",
        status: "resolved",
      },
      {
      ...
      },
      {
        ...
        description: "Temperature not reaching set point",
        status: "pending",
      },
    ])
    .returning();
